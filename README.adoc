...
[WARNING]
====
You can have troubles with using syntax highligting or with showing colours. Check that syntax setting is set up `:syntax on`.
Then you can have permitted only limited number of colors to be displayed. First there could be issue with your terminal
but as a first instance of help try to check colors to be shown in your vim instance `:set t_Co?` and if it's 8 or lower than 256
try to change it as `:set t_Co=256` (see http://vim.wikia.com/wiki/256_colors_setup_for_console_Vim#Vim_still_does_not_display_the_color_scheme_correctly)

Try to add these lines to your `.vimrc`
```
syntax on
if (&t_Co != 256)
  set t_Co=256
